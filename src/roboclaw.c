...
enum
{
	...
	ROBOCLAW_READ_ENCODERS_REPLY_BYTES = 12
};
...
static int encode_read_enc_m1(uint8_t *buffer, uint8_t address, uint16_t *cmd_crc16);
static int encode_read_enc_m2(uint8_t *buffer, uint8_t address, uint16_t *cmd_crc16);

static int16_t decode_read_main_battery_voltage(uint8_t *buffer);
static int32_t decode_single_encoder(uint8_t *buffer);
...
// Individual encoder read: reply is [4 value][1 status][2 CRC] = 7 bytes
static int encode_read_enc_m1(uint8_t *buffer, uint8_t address, uint16_t *cmd_crc16)
{
	uint8_t bytes = 0;
	buffer[bytes++] = address;
...
static int32_t decode_single_encoder(uint8_t *buffer)
{
	return (int32_t)decode_uint32_t(buffer);
	// byte 4 is status, bytes 5-6 are CRC — handled by send_cmd_wait_answer
}
...
struct roboclaw *roboclaw_init(const char *tty, int baudrate)
{
	...
	if (baudrate == 2400)
		...
		timeout_ms = ROBOCLAW_B460800_TIMEOUT_MS;
	else
...
/* Encode SETM1PID (28) or SETM2PID (29).
...
int roboclaw_encoders(struct roboclaw *rc, uint8_t address, int32_t *enc_m1, int32_t *enc_m2)
{
	...
	// Read M1
	bytes = encode_read_enc_m1(rc->buffer, address, &sent_crc16);
	if ((ret = send_cmd_wait_answer(rc, bytes, 7, sent_crc16)) < 0)
		return ret;
	*enc_m1 = decode_single_encoder(rc->buffer + bytes);

	// Read M2
	bytes = encode_read_enc_m2(rc->buffer, address, &sent_crc16);
	if ((ret = send_cmd_wait_answer(rc, bytes, 7, sent_crc16)) < 0)
		return ret;
	*enc_m2 = decode_single_encoder(rc->buffer + bytes);

	return ROBOCLAW_OK;
}

/* ── Encoder speed (GETM1SPEED cmd 18, GETM2SPEED cmd 19) ──────────────────
 * Reply: [speed:4][direction:1][CRC:2] = 7 bytes
 ...
 * Reply: [temp:2][CRC:2] = 4 bytes; temp value is °C × 10
 */
