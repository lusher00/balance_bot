...
static void decode_read_encoders(uint8_t *buffer, int32_t *enc1, int32_t *enc2)
{
	*enc1 = (int32_t)decode_uint32_t(buffer);
	buffer += 4;
	buffer += 1; // skip enc1 status byte
	*enc2 = (int32_t)decode_uint32_t(buffer);
	// enc2 status byte follows, then 2-byte CRC — both handled by send_cmd_wait_answer
}
