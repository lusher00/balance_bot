...
static int parse_args(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "i:p:u:b:t:m:B:d:qh")) != -1) {
        switch (opt) {
            case 'i':
                if      (strcmp(optarg, "ext")  == 0) input_mode = INPUT_EXT;
                else if (strcmp(optarg, "xbox") == 0) input_mode = INPUT_XBOX;
                else if (strcmp(optarg, "sbus") == 0) input_mode = INPUT_SBUS;
                else if (strcmp(optarg, "none") == 0) input_mode = INPUT_NONE;
                else {
            ...
            case 'u':  sbus_device = ext_uart_device = optarg;  break;
...
int main(int argc, char *argv[])
{
    ...
    switch (input_mode) {
        case INPUT_EXT:   uart_input_cleanup(); break;
        case INPUT_XBOX:  xbox_cleanup();       break;
        case INPUT_SBUS:  sbus_cleanup();       break;
        default:          break;
    }
