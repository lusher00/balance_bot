...
class Display:
    ...
    def _probe(self):
        """ACK-test the address on a descriptor WE own. Raises if absent.

        This exists because luma's i2c() opens /dev/i2c-N and then probes the
        address inside its own __init__, raising with the descriptor already
        open. There is no way to clean that up from outside: the constructor
        never returns, so we are never handed the object, and the only
        reference left is the one in the exception's traceback. Wrapping the
        call in try/finally does not help for the same reason -- there is
        nothing to close.

        (That is not a guess. The obvious version of this fix -- catch the
        exception and close `serial` -- was written first and measured: 300
        failed connects still leaked exactly 300 descriptors, because `serial`
        was None every time.)

        So the address gets tested before luma is involved at all, using
        smbus2 directly, with the descriptor in a local we close in `finally`.
        NOT write_quick(). A zero-data write is the probe i2cdetect uses and
        it is the obvious choice, but it cannot work on this SoC: the AM335x
        OMAP adapter declares I2C_AQ_NO_ZERO_LEN, so the i2c core rejects the
        transfer before it reaches the wire. This probe therefore failed 100%
        of the time, panel present or not, and the kernel logged

            i2c i2c-1: adapter quirk: no zero length (addr 0x003c, size 0, write)

        once per attempt -- 189 times in one boot of the log captured during
        the 2026-08-23 read-only event, at exactly this retry loop's 30 s
        back-off cadence.

        A one-byte write is not zero-length, so it passes the quirk check.
        0x00 is an SSD1306 command-mode control byte (Co=0, D/C#=0) with no
        command following it: a real panel accepts it and does nothing, and an
        absent one still raises OSError(EREMOTEIO), which is all this needs.
        """
        from smbus2 import SMBus
        bus = SMBus(self.port)
        try:
            bus.write_byte(self.addr, 0x00)
        finally:
            bus.close()
    ...
    def connect(self):
        ...
        # Absent panel -- by far the common failure -- is now rejected here,
        # before anything that could leak.
        self._probe()
        serial = None
        try:
            ...
            # climbs to EMFILE. Measured on this board with the panel
            # unplugged: after ~2h the process hit "[Errno 24] Too many open
            # files" on both /dev/i2c-1 and the systemctl subprocess, having
            # reached 164MB RSS and 28MB of swap on a 512MB machine. A status
            # display that cannot find its panel must idle cheaply, not
            # gradually take the board down with it.
    ...
    def ensure(self):
        ...
        try:
            ...
            ONCE.log("connect", logging.ERROR,
                     ...
                     "i2cdetect -y -r %d)", self.port, self.addr, str(e),
                     self.port)
            self._check_fd_leak()
            return False
    ...
    def _check_fd_leak(self):
        ...
        The failure mode was silent for two hours and then broke unrelated
        things (systemctl, i2c) with a confusing errno, so it is worth 5 lines
        to have it announce itself instead.
        """
        try:
        ...
        if n > 64:
            ONCE.log("fds", logging.ERROR,
                     ...
                     "this is a descriptor leak, not a display fault", n,
                     self._fail_streak)
    ...
    def _release(self):
        """Drop the device AND the bus handle behind it."""
...
def parse_args(argv=None):
    ...
    p.add_argument("--give-up-after", type=float, default=15.0,
                   help="exit non-zero if the panel has never been reached "
                        "within this many seconds (default: 15, 0 = never "
                        "give up and keep retrying forever)")
    ...
    try:
        while not stopping:
            ...
            if not display.ensure():
                ...
                # of time: `systemctl status bbb_oled` showed a green
                # "active (running)", hours of uptime and real CPU consumed,
                # while it had never once talked to a display. That is the same
                # lie netconsole told when it printed "network logging started"
                # with zero live targets, and it hid a dark panel for weeks.
                # A component that cannot do its job should say so where
                # someone will see it -- `systemctl --failed`.
                #
                # THE DEFAULT IS 15 s FOR AN ARITHMETIC REASON. The unit sets
                # StartLimitIntervalSec=120, StartLimitBurst=5, RestartSec=5,
                # so systemd only gives up if five starts fit inside 120 s:
                # 5 x (15 + 5) = 100 s, which does. Raise this much above ~19 s
                # and each attempt lasts long enough that the burst never
                # trips, systemd restarts forever, and the unit never shows as
                # failed -- exactly the behaviour this is meant to end. If you
                # want a longer grace period, raise StartLimitIntervalSec to
                # match rather than only changing this.
                if (not ever_connected and args.give_up_after > 0
                        and now - started_at >= args.give_up_after):
                    LOG.error(
                        "no panel at i2c-%d 0x%02X after %.0fs — exiting non-zero "
                        "rather than reporting healthy with nothing attached. "
                        "Check the wiring, then: sudo i2cget -y %d 0x%02X  "
                        "(errno 121 = bus fine, nobody home; 110 = a line is "
                        "held down). To retry after plugging it in: "
                        "sudo systemctl reset-failed bbb_oled && "
                        "sudo systemctl restart bbb_oled",
                        args.i2c_port, args.i2c_addr, now - started_at,
                        args.i2c_port, args.i2c_addr)
                    return 1

                # Back off while the panel is absent instead of reprobing at
                # the draw rate -- see Display.connect() for what that cost.
